...
// Elements carry motif-* classes so CSS keyframes (globals.css) can bring
// them to life: swinging bells, twinkling stars, glowing lanterns.
  ...
  return (
    <svg viewBox="0 0 400 160" fill="none" className={className} xmlns="http://www.w3.org/2000/svg">
      ...
      {[60, 140, 200, 265, 335].map((x, i) => (
        <g key={x} className="motif-bell" style={{ animationDelay: `${i * 0.35}s` }} opacity={0.92 - i * 0.05}>
      ...
      {Array.from({ length: 14 }).map((_, i) => (
        <circle key={i} className="motif-twinkle" style={{ animationDelay: `${(i % 5) * 0.4}s` }} cx={20 + i * 27} cy={14 + (i % 2) * 6} r="2.4" fill="currentColor" opacity="0.5" />
      ))}
    </svg>
  );
}
  ...
  return (
    <svg viewBox="0 0 400 200" fill="none" className={className} xmlns="http://www.w3.org/2000/svg">
      ...
      {Array.from({ length: 9 }).map((_, i) => {
        ...
        const len = 60 + (i % 3) * 26;
        return (
          <g key={i} className="motif-bell" style={{ animationDelay: `${i * 0.22}s` }} opacity={0.85 - (i % 3) * 0.08}>
...
export function BabyMotif({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 400 180" fill="none" className={className} xmlns="http://www.w3.org/2000/svg">
      ...
      <g className="motif-sway" transform="translate(90,50)">
  ...
  return (
    <svg viewBox="0 0 400 160" fill="none" className={className} xmlns="http://www.w3.org/2000/svg">
      ...
      {[70, 150, 230, 310].map((x, i) => (
        <g key={x} className="motif-sway" style={{ animationDelay: `${i * 0.3}s` }} transform={`translate(${x},${96 - (i % 2) * 6})`} opacity={0.9}>
      ...
      {Array.from({ length: 10 }).map((_, i) => {
        ...
        return (
          <g key={i} opacity={0.85}>
            ...
            <circle className="motif-glow" style={{ animationDelay: `${(i % 6) * 0.28}s` }} cx={x} cy={y + 6} r="6" stroke="currentColor" strokeWidth="1.2" />
          </g>
        );
      })}
    </svg>
